...
final class StorageService {
    ...
    ///
    /// **F2 RETIRED 2026-09-21 — many-to-many stays.** This function enforced
    /// *a part belongs to exactly one memory* (Tom, 2026-09-16) and refused a
    /// second live memory by throwing. `The descoping · synopsis.md` walks that
    /// back explicitly, and it is dated after the ruling it supersedes:
    ///
    /// > **Many-to-many stays.** This document claimed M:M existed *because*
    /// > clips were independently viewable. That was wrong. One photograph
    /// > legitimately belongs to both "our retirement trip" and "Judi at 70" —
    /// > that is reality, not an artifact of the removed UI.
    ///
    /// The costs were the argument: collapsing to one-source-one-memory is a
    /// CloudKit schema change with a Production deploy and a lossy migration,
    /// while keeping M:M costs nothing, **because the user never manipulates
    /// the relationship and never needs to know it exists**. Retiring the
    /// guard is correspondingly cheap: F2 was write-side only, so nothing
    /// migrates and no stored row changes meaning.
    ///
    /// What survives, and is still pinned by `PartsInSeveralMemoriesTests`:
    /// the **pair idempotency** below, and the requirement that a part with
    /// several memories reads, counts and re-attaches correctly.
