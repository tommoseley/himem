...
### A Single-Variable Intervention Needs Its Control (Mandatory)

**Changing one variable and seeing the symptom go is not evidence that the variable was the cause. It is evidence that the symptom is gone.** Those are different claims, and only the second is measured. **Before naming a variable as the cause, run the control: put the variable back and see whether the symptom returns.** If it does not, the variable is exonerated and something else moved.

- **The confound is usually TIME, and it is invisible because nothing in the report names it.** A gap between the failure and the retest is itself an uncontrolled change — an OS update, a reboot, a dependency, a cache expiring, an intermittent that simply is not present today. **State the gap's length and its contents in the report**, next to the result, so a reader can see what else could have produced it.
- **A green run answers "does it pass now?", never "was my hypothesis right?"** The question you asked is not the question the run answered — see § Measurement Discipline, the fifth class. The stronger your mechanism sounds, the more a single green reads as confirmation, and the less likely anyone asks for the control.
- **The cost of skipping it is not a wrong sentence in a log — it is a fix that works for the wrong reason.** Such a fix is **indistinguishable from a fix that works, until the real cause returns** — at which point the fabricated one is load-bearing, credited, and defended. It also carries its false origin story in a commit message, where the next reader inherits it as fact.
- **Where the control is cheap, it is not optional.** One extra invocation is the whole cost.

*Origin: 2026-09-10, the watch test-runner install failure. Five consecutive runs had died at `installApplication` with `Invalid device state` / `Mach error -308 — (ipc/mig) server died`. `~/Library/Logs/CoreSimulator/CoreSimulator.log` — an artifact no prior session had read, spanning weeks — showed the `-308`s attached to **clone** devices, and the schemes carried an exact asymmetry: the watch scheme sets `parallelizable = "YES"` on both testables and clones up to eight simulators, while the phone scheme (which installs its runner without trouble) sets it nowhere and produced `Clone N of iPhone 17 Pro` **zero times in 28,951 lines**. A run with `-parallel-testing-enabled NO` came back **34/6, 0 failed, runner installed** — and it looked exactly like confirmation. **The control run, flag flipped back to `YES` and clones demonstrably in use, returned the identical 34/6, 0 failed.** The hypothesis was disconfirmed by the one command that could disconfirm it. What had actually moved was the environment: macOS updated **2h34m after the failing session's last commit** and again eight days later, with a reboot between — a candidate, not a finding, and untestable because the failure no longer reproduces.*

***The counterfactual is why this is a rule.*** *Without the control, `-parallel-testing-enabled NO` would have shipped. It would have **passed forever**, **taken credit for the repair**, **silently slowed every watch gate**, and **carried a fabricated origin story in its commit message** — while the real cause stayed unnamed and free to return. The asymmetry was real; it simply was not the mechanism. Finding a true difference between a working case and a failing one is not the same as finding the cause, and the difference between those two is one invocation.*

**Secondary instance, true independent of how the above resolved — an experiment that cannot reach the thing it exonerates.** The same investigation had inherited three eliminated causes, and two of them were retired by experiments structurally incapable of testing them: *"not stale device state — an in-place erase of both halves didn't help"* (erasing the **pinned** devices cannot affect **clones**, which are minted fresh every run) and *"a watch simulator boots and survives fine alone"* (booting a device directly never creates a clone). Both statements were accurate about what they did; neither reached the hypothesis it was taken to close. **State an elimination's REACH, not just its result** — *what would this experiment have done if the hypothesis were true?* An elimination that could not have failed has eliminated nothing.
...
- **A BANNED-WORD GUARD CHECKS VOCABULARY, NEVER TRUTH — and a sentence can be entirely free of the banned words and still tell the user something false.** This is the copy-test counterpart of the wrong-quantity class in § Measurement Discipline: the assertion is well-formed, it passes for the right reason, and it is answering a different question than the one that matters.
  - **The tell is a guard phrased as a list of words rather than as a claim the sentence makes.** *"Never says 'failed'"* is a rule about diction. *"Never asserts a cause we cannot observe"* is a rule about truth, and only the second could have caught this.
  - **Ask what the app can actually KNOW**, then forbid every sentence that claims more. The export can observe that a file has not arrived. It cannot observe why — not the network, not the account, not iCloud's queue — so every explanation it could offer is a guess, and a guess stated as fact in a completion line is indistinguishable from a diagnosis.
  - **A wrong cause is worse than no cause, because it names a useless action.** *"Try again when you're on Wi-Fi"* tells someone already on Wi-Fi that the remedy has been tried and the problem is elsewhere. The honest version — *"still in iCloud and haven't come down yet — run this again in a few minutes"* — reports the state and names the action that works.

*Origin: 2026-09-21, the recordings export's first device run.* `neverSaysFailed` *looped over five outcome shapes asserting that none contained "fail", "error" or "unable". It passed, correctly, on the sentence* **"Saved 130 recordings. 108 couldn't be downloaded from iCloud — try again when you're on Wi-Fi."** *Tom was on Wi-Fi. The guard was right about its rule and blind to the defect, and the defect was the one thing in the sentence the app had no way to know. Remedy shipped with it:* `theCopyNeverAssertsACause` *forbids "wi-fi", "offline", "connection", "network" and "signal" across every outcome shape — but the words are the mechanism, and the rule is the comment beside them.*
...
- **A SOURCE SCANNER'S SELF-TESTS MUST INCLUDE A MALFORMED INPUT**, alongside the offender and the clean case. Its input is not what any test constructs — **it is every file the walk reaches**, so the shapes it must survive are whatever anyone writes next. A scanner that mis-reads under-reports, which a self-test catches; a scanner that **traps kills the test HOST**, and nothing catches that. Blast radius is the whole run, not one assertion.
  - **The tell is a self-test set that covers offender / fix / near-miss and never asks "what if the shape is degenerate?"** Three self-tests did exactly that and the fourth case took down 110 suites.
  - **Classify a slice by what its bound GUARANTEES, not by whether it has one.** `lines[start..<min(i + 20, count)]` traps just as readily if `start > i + 20`; it is safe only because `start = max(0, i - 6)` is provably below `i`. Reading the `min()` as proof of safety is the over-report shape pointed at the audit itself.

*Origin: 2026-08-25. `isOffendingClosureLabelOutsideDecoration` sliced a label body as `lines[(labelIdx + 1)...close]`. A button whose label OPENS AND CLOSES ON ONE LINE — `Button { a += 1 } label: { pill("A") }` — makes `close == labelIdx`, so the range inverts and Swift traps:* **"Range requires lowerBound <= upperBound"**. *A `#if DEBUG` probe added for an unrelated isolation supplied that shape and the run reported* **199 failures across 110 suites for one defect**. *It was attributed for a full day to "a `#if DEBUG` view that no test constructs is taking down the host" — true, and irrelevant:* **nothing needed to construct it, because being read as source text was sufficient. A scanner's input is not what runs; it is what exists** *(Tom). The audit that followed found the class smaller than feared — 40 source-walking files, 30 slice sites, exactly one trap-capable — so the remedy is this rule plus one bounded helper, not a retrofit of 29 provably-safe sites. **"Three scanners" was a count inside one file**; getting that wrong in the direction of fewer would have left the class unexamined.*
