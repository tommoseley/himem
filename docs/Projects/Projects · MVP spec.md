...
- **Minimum threshold: 3 memories.** Below that the input is too thin to summarize honestly. The button shows but is disabled, with a quiet reason line.
...
1. **Never run** (≥3 memories): “Find the thread · 1 assist” card with a single **Run** button, AI blue.
2. **Below threshold** (<3 memories): same card, button disabled, sub-line reads “Add a few more memories first.”
...
- **Same constraints as paid runs.** ≥3 memories, reads summaries + titles + topics + dates (not raw transcripts), produces one Honest-Label paragraph, doesn't mutate individual memories.
