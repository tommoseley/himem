...
- **Clusters are pre-accepted proposals**, shown only when confident. Per-card affordances are **quiet secondaries**: *Adjust* (blue) and *Not together* (dismiss → clips fall back to loose).
  - **Adjust = expand-in-place + Remove (subtractive) (locked 2026-07-15).** Tapping *Adjust* (or the card body) expands the card in place into a minimal editor: every member clip renders as a `ClipAtom` row, each with a subtractive **Remove**. Removing a clip **sets it aside** — it drops to a quiet *"Not in this memory"* section within the card, reversible via **Add back** — and it falls back to loose on commit (a per-clip *"Not together"*). Trims are **provisional-and-reversible until commit**: the expanded card *is* the review, *Keep these · N* is the decision (Model A — no new store, no proposer change; a removed clip is simply not committed, so it stays loose). **N updates live** as clips are trimmed; a cluster trimmed to empty drops from the count, and an all-empty batch **disables** the commit (never an empty memory). **Out of scope, still post-v1:** pulling a clip *into* a cluster; any bidirectional/reordering editor. This **supersedes the earlier "post-v1 cluster editor" placeholder** — the minimal subtractive editor ships in v1.
