...
**What "opened" clears (locked July 19 2026 — the container rule).** *Opening the container you're shown marks what's in it as seen.* Concretely: **a clip leaves New when opened** (its Clip Detail / Clip Editor is shown), and **a session leaves New when opened — opening a session card batch-marks *every* clip in it seen in one act.** The per-clip alternative (a 5-clip session stays New until you tap into each of its five clips) is rejected: it turns New into a chore to zero out — the guilt-inbox friction we explicitly reject (`Kingfisher · North Star.md`, calm-by-default / workbench-not-queue). New answers one question — *"should I look?"* — and the session view surfaces every clip's content when you open it, so *"opened the session"* honestly is *"saw these."* This is why review state must be per-clip *stored* but is *set* at whatever container the user opened: standalone clip → that clip; session card → all its clips.
