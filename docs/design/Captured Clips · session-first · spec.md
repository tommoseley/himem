...
- **The collapsed card shows a read-only teaser + an honest "Show all" disclosure (locked July 17 2026).** Collapsed, the card shows the blue signal strip, the cluster title, and a short read-only teaser (time · first-line per clip). The disclosure control reads **"Show all N ⌄"** (→ **"Done ⌃"** when expanded) — a label that says what it does (open to read/edit), replacing the earlier **"Adjust ⌄"** (which named editing, not opening, and left the full view behind a word most users wouldn't read as "see more"). **The whole card is the expand/collapse target** — head, title, and teaser rows all toggle it; "Show all" is the visible cue so the affordance is honest, not hidden. Editing/trim controls (✎, ⊖) and the Compact/Full toggle live *inside* the expanded state, not on the collapsed teaser. *Not together* (dismiss the whole cluster → clips fall back to loose) stays beside the disclosure.
