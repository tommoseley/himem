...
- **Adjust = expand-in-place + subtractive trim (Model A, locked July 15 2026).** Tapping *Adjust* (or the cluster name row) expands the card in place to show all N clips as **`reflectiveCompact` rows in a single-open accordion** (reusing the Memory Detail long-memory pattern — glyph · time · one-line preview · play · chevron; tap a row to expand its full transcript in place, opening one collapses the prior, global single-open across the editor). Stacked full transcripts are *not* used — the accordion, not an 8-line preview, is what keeps 11 clips scannable; the expanded transcript is read-only (view-and-trim; row-tap → Clip Detail stays post-v1). Each row carries a subtractive **Remove** (reusing the Clip-model §187 *Remove* vocabulary). A removed clip **sets aside** in place — its content dims but stays visible under a divider with a full-strength **Add back** control (≥44px; status/affordance is never opacity-alone), so the trim is *reversible*, not a vanish. **The trim is provisional until commit:** removed clips are simply not committed, so they stay loose after *Keep these*; nothing is persisted or told to the proposer (no clip-granularity set-aside — Model B was considered and rejected as over-build for v1, since after commit the kept clips leave and the maximal grouping can't reform). **`Keep these · N` updates N live** as clips are trimmed; **trim-to-0 disables commit** ("Nothing found to keep"). **Out of scope (post-v1):** pulling a clip *into* a cluster, any bidirectional membership editor, and **row-tap → Clip Detail** from inside the editor (rows are view-and-trim only in v1 — known, not a gap). Supersedes the earlier `// post-v1 cluster editor` placeholder, which was an implementer inference, not a design decision.
