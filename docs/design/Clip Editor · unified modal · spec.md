...
- **Source glyph on the metadata line (Finding 2, locked July 16 2026).** The timing line is preceded by a small capture-source glyph — `applewatch` for a Watch clip, `iphone` for a phone clip — per the source-agnostic rule (source is per-clip *metadata*, a small glyph, never a headline; `CLAUDE.md` §Phone). Available for bench (`InboxClip`) clips, which carry `source`. **Placed/managed clips (`MediaReference`) have no source field yet**, so they show no glyph until `MediaReference.sourceDevice` ships — a CloudKit-synced schema change bundled into the next deploy (deferred, not dropped: "inbox now, managed field later"). Siri capture doesn't produce clips yet; its glyph ships with that source.
