...
- **The screen walkthrough lives under the ?** hub as **"Show me around"** (renamed from "Take a tour of the screen", F2b · 2026-07-26 — one label, matching the empty-home chip; the row is now an action that restores the anchored per-tab coachmarks, not a navigation to a stub), replayable any time, and is *offered* on the empty home via the same quiet **"Show me around"** affordance — never auto-forced between the user and capture.
...
6. **Recoverable, and the recovery is taught (F2b · locked 2026-07-26).** A coachmark dismissed in one gesture must be reachable again, and restoring it is useless if nobody knows it's possible — so both halves ship together. **(Part 1 · the Learn hub restores them.)** The **? → Learn hub → "Show me around"** row is an *action*, not a navigation: it clears every per-tab seen flag and re-fires the **current tab's** coachmark once the hub closes. It is **stateless** — always present, never gated on whether coachmarks have been seen, no "seen" checkmark. It deliberately does **not** reset the session counter (guard #4): the user is past the second-session gate, so a manual restore re-coaches *now*, and the other tabs re-coach on their next first visit, exactly as on a fresh install. **(Part 2 · the exit teaches the way back.)** Every coachmark card — since each tab's coachmark is a single self-contained card, this is its final beat — carries a quiet closing label: **"That's it! You can bring this tour back any time from ? → Show me around."** Copy is design-authority (exact string). It is a **quiet label** (ink2/ink3, no border, **not tappable**) — information, not an action; it must never read as a button, and it is **not** a toast or banner. Shown **every** time the card is dismissed, not just the first. *(Testable seam: the seen-flag reset + current-tab re-arm — `CoachmarkRecoverabilityTests`. Card copy/placement is visual — device-verified.)*
