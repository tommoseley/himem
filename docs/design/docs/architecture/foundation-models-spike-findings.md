...
**Honest Label remains the product standard.** On Free, the on-device model produces a draft *toward* that standard; the user completes the label through review and edit. On Plus, the frontier model meets the standard more reliably and adds the Connect features the on-device model cannot reach. The two tiers differ by quality and reach, not by automation alone.
