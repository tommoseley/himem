...
- **The palette is for matching, not insertion (locked July 18 2026).** The `existingTopics` / `existingMentions` lists are handed to the organizer for **spelling and de-duplication only** — to *prefer* an existing label over coining a near-duplicate. They are **never a set to add from.** The organizer must only name a topic or mention that *this* memory's own clips actually refer to; a palette entry the clips don't mention must not appear. An unguarded palette caused real injection on the on-device path (the July 18 calibration: *"Mmmm, pears"* returned a hallucinated **Darlene** mention purely because she was in the palette). The guard is now carried explicitly in the on-device prompt, and the QA calibration set (fixtures 2 / 4 / 5) measures it. This is the palette contract restated, not a new rule — reuse-to-dedupe was always the point; injection was always out of scope.
...
The June 5 spike validated the on-device model as **good enough to be the default Free layer** — 12/15 fixtures clean, the rest hand-editable. But a handful of failure classes survive every prompt approach and are **model-capability ceilings, not prompt bugs** (classes 1–3 from the June spike, 4–5 from the July 18 calibration):
...
4. **Subject-out on pure-observation (added July 18 2026 calibration)** — a photo-only memory with no first-person voice still renders *"You're capturing a photo of…"* instead of leaving the owner out, despite an explicit subject-out rule in the prompt (§3 Voice). QA calibration fixture 5. Frontier/Plus is expected to clear it.
5. **Stray palette mention (added July 18 2026 calibration)** — an occasional single unreferenced name from the `existingMentions` palette slips into the mentions list even with the "matching-not-insertion" guard (§2c). QA calibration fixture 4.

Classes 4–5 are hand-editable in the Draft-organized review like the rest, and — per the July 18 ruling — are **not to be chased further on the 3B on-device model**; they are the reason Free ships an editable draft, not a guarantee, and the frontier/Plus path should clear them.
...
- **Pure-observation clips** (sunset photo, no audio): leave the subject out entirely. *"A sunset over the ridge."* No "you" needed — these summaries render the same on share. *(Carried in the on-device prompt as an explicit subject-out rule since July 18 2026; the 3B model doesn't fully honor it yet — see §2b class 4.)*
