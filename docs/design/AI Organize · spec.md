...
- **Plus — automatic, frontier.** The **first** pass happens **for you**, on capture, on a frontier model — plus the deeper connective work (mentions across memories, related memories, project suggestions). When a Plus user captures offline, the validated on-device draft serves immediately as that first pass. **On reconnect the draft stays as-is — HiMem never silently repolishes or overwrites a completed pass.** If the user wants the frontier take, they tap **Reorganize with AI ✦** (the same explicit re-run Free uses). *This is a manual/explicit model, revised July 20 2026 (RH-5): the earlier "frontier polish lands silently on reconnect" is retired — a completed organize sticks until the user re-runs it, per the no-silent-rewrites rule (`feedback_no_auto_reprocess`). Silent rewrites break trust; the user always initiates a re-pass. (`nextSteps` is cut from v1 — see §2/RH-6.)*
...
**Plus offline capture reuses these states, no new ones (manual model, revised July 20 2026 · RH-5).** When a Plus user captures offline, the on-device draft enters `organized: true, reviewed: false` immediately and **stays** — the pass is complete. There is **no silent frontier repolish on reconnect**: HiMem never re-runs or overwrites a completed organize behind the user's back (`feedback_no_auto_reprocess`). The user gets the frontier take by tapping **Reorganize with AI ✦** — the same explicit re-run Free uses, which lands as a fresh reviewable draft (the previous summary stays visible until it commits). The UI layer still knows nothing about which model produced a given output; it only ever renders a completed pass and a user-initiated re-pass. *(This retires the earlier "frontier polish lands silently on reconnect / one pipeline, two backends" auto-repolish language. Spike §7.1.)*
