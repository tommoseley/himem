...
So there is a **mandatory code-side Honest-Label verification gate** on the on-device path (`HonestLabelGate` + `ProcessingEngine.gateOnDeviceResult`): after the on-device summary returns, it is verified against the source clip text — **any proper name / concrete entity in the summary not present in the clips is a violation** → retry once → then fall back to a **constrained extractive summary** drawn from the clips, which cannot introduce a name the source lacks (*"say less before saying false"*). Deterministic and OS-independent, so it holds no matter how far Apple's model drifts. The gate is **on-device-only** — the frontier/Plus path is stable and honest and is not gated. The QA-panel probe now catches **any** invented proper noun (not a fixed banned-string list, which missed "Albert").
