...
# PART 0 · Design authority (read first)

**The designs and specs in this project are authoritative, not suggestions.** They are decisions, arrived at deliberately. Whoever implements them — Claude Code or anyone else — builds what they specify. The implementer's latitude is ***how*, never *what*:** view structure, state plumbing, file organization, naming of internal code, the mechanics of making the specified behavior work. The behavior, copy, verbs, ontology, layout intent, and interaction model are already decided.

- **Raise concerns, don't deviate.** If a spec looks wrong, impossible, contradictory, or costly, **stop and say so** — clearly, with reasoning — and wait for a ruling. That is not just allowed, it's wanted (see "Push back honestly"). What is *not* allowed is quietly building something different, "improving" a design in passing, or resolving an ambiguity by inventing a new *what*. A raised concern gets a decision; a silent deviation gets caught in review and costs a round-trip.
- **Ambiguity is a question, not a license.** If the spec doesn't cover a case, ask. Don't fill the gap with a guess and ship it.
- **Two classes of edit** (per `HiMem · Locked Decisions.html`): a *coherence fix* — wording/mechanics that make the build match an existing decision — is fine to apply. A *vocabulary, architecture, or principle change* — anything that alters a decision itself — requires explicit approval, even when it seems obviously better.
- **Definition of done:** first *"does this express the design as specified?"*, then *"does it work?"* — in that order. A working build that deviates from the design is not done; it's a regression to be reverted.

This exists because the expensive failure mode here is not "CC couldn't build it" — it's "CC built something subtly different and we found out three screens later." One raised concern is cheap. One silent deviation is a day.
...
**Captured Clips is the consolidation seam for ad-hoc captures**, regardless of which surface produced them. The user is at an intake conveyor — deciding which of these raw thought streams become actual Memories. The **session** — not the individual clip — is the right primary unit here: a session is the natural grouping of what the user is moving through, and any session *may* become a memory (though it needn't).
...
- **Clips is the first-class evidence surface** (retires the standalone "Captured Clips" window). Default view = AI suggestions + everything not-yet-connected (incl. clips still downloading from the phone), no architecture word exposed. **The filter is TWO independent axes, never one control (locked July 12 2026; status axis redefined July 18 2026):** a **status** lens and a **type** filter (**All / Voice / Photos / Video / Notes** — media), so "new videos only" is expressible. Type is a neutral chip row (secondary refinement). **The status lens is New · All · Unconnected — chip order `All · New · Unconnected`, New default on entry (revised July 18 2026, supersedes the earlier New⟷All = connected-ness definition; label "Loose"→"Unconnected" July 19 2026; order All-left/New-default July 19 2026):** a clip has **two orthogonal properties**. **Review state (stored):** **New** = `reviewed==false` (never opened), **Reviewed** = `reviewed==true` ("opened by you" — the *act*, not the outcome; you may review-then-attach or review-then-leave-unconnected). **Connection count (derived from edges):** **Unconnected** = `connectionCount==0`. "Attached" is a *relationship* (≥1 edge), not a lifecycle state. **New now means *unseen*, not *unconnected***, which finally aligns the filter with the tab dot (both = unseen arrivals). **Unconnected** covers never-used ∨ previously-attached-then-deleted ∨ manually-detached — the cleanup lens. *Why the change: deleting a memory returns its clips to the bench (Let Go lock), and under the old connected-ness definition those re-appeared as "New" though they're not new — previously-shaped, now unconnected. Splitting the review-state axis from the connection axis makes each honest.* *Label note: the chip reads **"Unconnected"** (not "Loose"/"Available") — consistent with the status sheet's "Not yet connected" and "Capture once, connect many"; a filter label must name what it selects, and "Available" is imprecise since every clip is available.* **`reviewed` is persisted per-device, not synced (decided July 18 2026)** — a local field on `InboxClip` (manifest) and on bench `MediaReference`; **no CloudKit deploy** (the V7 batch already shipped). Review state is a noise-reduction signal ("clear the New lens of what I've eyeballed"), not load-bearing, so per-device is honest for v1 (the tab dot already behaves this way); true cross-device review-sync is a property of the post-v1 bench→MediaReference unification, solved there once. Open-then-close and attach-then-delete both yield reviewed=true/0-connections, indistinguishable from edge history, so "has this been opened" is a stored field; connection count *is* derived. **"Orphaned" is architecture-only — the UI only ever says "Loose."** The library-not-inbox framing: a clip library has *recently-acquired / available / referenced*, not "old new books." **Capture returns to Clips.** Tapping a clip opens the clip as the primary object (transcript · media · date · **Referenced in: [memories]** · Projects).
- **Unconnected-clip cleanup lives in the Unconnected filter (locked July 18 2026; label July 19 2026).** The bench never cascade-deletes clips when a memory/project is deleted (the Let Go lock preserves them). Instead, the **Unconnected** filter surfaces every zero-connection clip and supports **multi-select → Delete / Add to a memory** (Photos-style; reuses the Sort multi-select). This is the "clear" path for the pile of no-longer-needed clips a deletion leaves behind — Delete here is clip-deletion ("Delete this Clip", destroys the atom). **No branching at deletion time:** Let Go/Delete-project add no clip checkbox or modal — the honest sentence ("The clips stay…") is enough; cleanup is a separate, deliberate act on the bench.
- **New-arrival status is a dot on the Clips tab, not a banner** (banner retired July 10 2026 — it predated the three-tab model, when Captured Clips was a hidden window; now Clips is an always-visible tab, so a banner pointing at it is redundant chrome on the reflective Memories surface). The dot answers *"should I look?"* — **presence, not a count** (a number implies an obligation to zero-out, the guilt-inbox we reject). The dot represents **new, unseen arrivals** and **clears when the user opens Clips**. *(Reconciled July 18 2026: with the status lens redefined so **New = unseen**, the dot and the New filter now track the same thing — unseen arrivals — rather than disagreeing. "What's still unconnected" is carried by the separate **Unconnected** filter, no longer by "New.")*
...
- **Active Navigation Tap (status sheet) — required for v1, not optional.** First tap on a tab navigates; tapping the *active* tab scrolls to top if scrolled, and — when already at top — presents a **status sheet** for that section. On Clips the sheet shows *new arrivals by source (the full source roster shows every time, including sources at 0 — reversed July 12 2026 from the earlier omit-zeros rule; a stable, scannable list beats one that changes shape, and "0 from Siri today" is real information), what's processing (downloading / organizing), and what's **not yet connected** (the count of unshaped clips)* plus quick filter shortcuts — **no census/vanity counts** (no "247 referenced"; the sheet is *what needs me*, not *how much I have*). Section label is **"Not yet connected"**, not internal jargon like "loose / available to shape." This is a required build; what's optional is only the *user's* discovery of it — status also lives in the dot + default filter, so the app works for someone who never taps twice. See `Kingfisher · North Star.md` · "Calm by default, density on demand."
...
- **Project** is an owner-created, long-running effort with a name and a goal field ("What are you building toward?"). It **connects** memories (many-to-many) — it does not contain or own them. See `Projects · MVP spec.md` for the full ruleset.
