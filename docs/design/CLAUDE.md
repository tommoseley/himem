...
- **Tier:** Plus or Founders required *beyond the first run*. Free gets **1 active project + 1 starter Project Assist run** (separate from the 3 starter memory assists). The starter run is silent — the first tap of "Find the thread" just works; the upsell fires only on the second attempt. Studio (post-MVP) reads raw fragments + cross-project + structured output + export.
