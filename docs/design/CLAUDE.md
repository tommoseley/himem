...
- **Audio format — transcode before transfer (locked 2026-07-14).** The watch transcodes every clip to **mono · 16 kHz · AAC (`.m4a`) before `transferFile`; it never ships the raw recording.** The hardware input is 3-ch/48 kHz/Float32 PCM (~576 KB/s) — a 59 s clip is ~33 MB raw vs ~230 KB compressed (~144×), and the raw payload takes minutes over WatchConnectivity (the ~50× sync-slowness bug). Whole-file transcode **after stop, never per-callback** (per-callback resampling in the record tap starves the resampler → silence; July 5 reverts). **Explicit mono downmix** — `setVoiceProcessingEnabled(false)` does not yield mono on device. Guard: the file handed to `transferFile` is mono/16k/AAC — that assertion failing is the bug. Full detail: `Watch · spec.md §2` + `docs/architecture/2026-07-14-watch-audio-compression.md`.
- **Transport — WatchConnectivity, permanently (locked 2026-07-14).** The watch **never** writes to CloudKit or an iCloud container. It hands clips to the phone over `transferFile`; **the phone is the sole iCloud writer** (media → iCloud Files, metadata → private DB), off the capture path. The "watch uploads to CloudKit" idea is **retired, not deferred** — compression makes WCSession fast enough, and keeping the watch off iCloud preserves the phone-as-sole-writer custody model.
...
- **Clips is the first-class evidence surface** (retires the standalone "Captured Clips" window). Default view = AI suggestions + everything not-yet-connected (incl. clips still downloading from the phone), no architecture word exposed. **The filter is TWO independent axes, never one control (locked July 12 2026):** a **status** toggle (**New ⟷ All** — connected-ness) and a **type** filter (**All / Voice / Photos / Video / Notes** — media), so "new videos only" is expressible. Status is the ochre two-state toggle (the primary lens); type is a neutral chip row (secondary refinement). The earlier single row (`New · All · Voice · Photos · Notes`) is retired — it conflated the axes and had no Video. **Capture returns to Clips.** Tapping a clip opens the clip as the primary object (transcript · media · date · **Referenced in: [memories]** · Projects).
