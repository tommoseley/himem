...
- **Audio-format invariant (locked 2026-07-14).** Every clip handed to transfer is **mono / 16 kHz / AAC**. Raw multi-channel PCM must never leave the watch. The pre-transfer artifact asserts `channelCount == 1 && sampleRate == 16000 && codec == AAC`; a transferred clip's `bytesPerAudioSec < ~8000`. This is a **real assertion test, not a log line** — a log didn't catch 33 MB clips for weeks; an assertion fails on dogfood #1. The assertion pairs format with **non-zero output energy from a ≥3ch source fixture**, so *format-correct-but-silent* fails (P0 2026-07-15). 3-channel device audio is downmixed to mono by a **manual per-frame channel average**, then a mono→mono resample — **not** by `AVAudioConverter`'s built-in downmix, which has no matrix for an unlabeled/discrete >2ch layout and emits pure silence. Never hand N-channel straight to the converter.
