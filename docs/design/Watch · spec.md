...
- **Audio-format invariant (locked 2026-07-14).** Every clip handed to transfer is **mono / 16 kHz / AAC**. Raw multi-channel PCM must never leave the watch. The pre-transfer artifact asserts `channelCount == 1 && sampleRate == 16000 && codec == AAC`; a transferred clip's `bytesPerAudioSec < ~8000`. This is a **real assertion test, not a log line** — a log didn't catch 33 MB clips for weeks; an assertion fails on dogfood #1. The assertion pairs format with **hottest-channel output energy from a ≥3ch source fixture**, so *format-correct-but-silent* fails (P0 2026-07-15). Multi-channel device audio is downmixed to mono by **extracting the hottest channel** (revised 2026-07-15 from a per-frame average: the device's channels were live but uncorrelated, so averaging cancelled misaligned peaks and lost ~2×), then a mono→mono resample — **not** by `AVAudioConverter`'s built-in downmix, which has no matrix for an unlabeled/discrete >2ch layout and emits pure silence. Never hand N-channel straight to the converter. This downmix is **retained as defensive, tested code** even though the capture-mode fix below now yields mono on device.
- **Capture session-mode invariant (locked 2026-07-15).** The watch records in **`AVAudioSession.Mode.default`, never `.measurement`**. `.measurement` minimizes system input processing **including input gain**, which left the watch mic at ~−40 dBFS — clips arrived **silent and untranscribable** (dogfood: `in_peak` pinned ~0.01 regardless of spoken loudness), and it was *also* selecting the raw 3-channel hardware input. `.default` applies normal gain **and** resolves to processed **mono** — one change fixes the level bug and dissolves the 3-channel downmix problem at the source (dogfood 2026-07-15: `in_peak` 0.1, full-coverage transcripts). The mode is the constant `WatchAudioSessionConfig.recordMode`, guarded by `WatchAudioSessionConfigTests` (a config assertion — real input energy needs hardware, which the `[Amp]` transcode log covers device-side).
